...
\author{
Kunal Achintya Reddy S\textsuperscript{1},
Anthireddy Raja Narsimha\textsuperscript{2},
Komrewar Rishieth\textsuperscript{3} \\[0.5em]
\textsuperscript{1}Department of Computer Science, Amrita Vishwa Vidyapeetham \\
\textsuperscript{2}Department of Mechanical Engineering, Amrita Vishwa Vidyapeetham \\
\textsuperscript{3}Department of Mechanical Engineering, Amrita Vishwa Vidyapeetham \\[0.5em]
\texttt{kunalachintya@gmail.com} \\
\texttt{rajanarsimha21@gmail.com} \\
\texttt{rishiethkomrewar@gmail.com}%
...
twin. Table~\ref{tab:theory} previews these guarantees, the assumption each
depends on, and where each is established.

\begin{table*}[!t]
\renewcommand{\arraystretch}{1.25}
\caption{Summary of the theoretical guarantees instantiated for the cell twin}
\label{tab:theory}
\centering
\begin{tabular}{@{}clll@{}}
\toprule
Result & Guarantee & Governing assumption & Section \\
\midrule
Theorem~\ref{thm:wellposed} & global existence, uniqueness, invariance of $\mathcal{D}$ & locally Lipschitz $F$; GSH-independent scavenging $s_0>0$ & Section~\ref{sec:model} \\
Theorem~\ref{thm:filter} & sequential update equals the exact posterior & conditional independence of observations given $\theta$ & Section~\ref{sec:inference} \\
Theorem~\ref{thm:contraction} & posterior contracts at rate $O(n^{-1/2})$ & positive-definite Fisher information (Assumption~\ref{ass:ident}) & Section~\ref{sec:inference} \\
Theorem~\ref{thm:eig} & EIG equals mutual information and is nonnegative & compact design set, continuous EIG & Section~\ref{sec:design} \\
Theorem~\ref{thm:reduction} & information-optimal design needs the fewest experiments & Loewner domination of Fisher information & Section~\ref{sec:design} \\
\bottomrule
\end{tabular}
\end{table*}

Section~\ref{sec:model} presents the model and proves well-posedness;
...
each figure and number is regenerated by a single script. Table~\ref{tab:results}
consolidates the headline quantities before they are discussed in turn.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.25}
\caption{Summary of empirical results (all generated from the engine)}
\label{tab:results}
\centering
\footnotesize
\begin{tabular}{@{}p{2.5cm}p{4.0cm}c@{}}
\toprule
Quantity & Result & Ref. \\
\midrule
Literature calibration (17 toxins) & 17/17 within one log; median fold-error $1.00\times$; Spearman $\rho=0.996$ & Fig.~\ref{fig:calibration} \\
Mechanism-only ordering & $\rho\approx0.88$ before any calibration & Fig.~\ref{fig:calibration} \\
Bayesian IC$_{50}$ (rotenone) & $16.8\,\mu$M (90\% CI $15.8$--$18.1$); $\hat R=0.999$; shrinkage $0.94$ & Fig.~\ref{fig:bayes} \\
Parameter recovery & true potency $0.70 \rightarrow 0.67$ recovered & Fig.~\ref{fig:bayes} \\
Online assimilation & severity $0.70\rightarrow0.70$; CI width $0.15\rightarrow0.06$ & Fig.~\ref{fig:assim} \\
Tissue selectivity & cardio-/nephro-selectivity, tumor resistance reproduced & Fig.~\ref{fig:selectivity} \\
ML surrogate & $R^2=0.93$ overall; $\ge0.96$ on graded endpoints & \S\ref{sec:results} \\
JAX vs.\ stiff solver & $<2\times10^{-2}$ viability error, all toxins & \S\ref{sec:inference} \\
\bottomrule
\end{tabular}
\end{table}
